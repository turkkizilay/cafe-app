...
function AuthErrorScreen() {
  ...
  useEffect(() => {
    // Hash aus URL entfernen, damit kein Refresh den Fehler wieder zeigt
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#1C1917' }}>
      <div style={{ background:'#fff', borderRadius:14, padding:'40px 36px', maxWidth:440, textAlign:'center', boxShadow:'0 20px 40px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ display:'flex', gap:10, justifyContent:'center' }}>
          ...
          <button
            onClick={() => { supabase.auth.signOut(); sessionStorage.removeItem('cafe_session_active'); localStorage.removeItem('cafe_no_remember') }}
...
export default function App() {
  ...
  const handleAutoLogout = useCallback(async (reason) => {
    // Protokoll VOR signOut — danach ist auth.uid() null und log_activity würde ablehnen
    try {
      await logActivity({
    ...
    setProfile(null)
    if (reason === 'timeout') {
      // Toast kann hier nicht mehr funktionieren weil App neulädt → sessionStorage-Marker
  ...
  const fetchProfile = useCallback(async (uid) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('id', uid)
        .maybeSingle()  // maybeSingle statt single → kein Fehler wenn nicht gefunden
  ...
  // ── "Angemeldet bleiben" — beim App-Start prüfen ─────────────────────────
  useEffect(() => {
    ...
    if (noRemember && !sessionActive) {
      // Browser wurde neugestartet, "Angemeldet bleiben" war nicht aktiviert → ausloggen
      supabase.auth.signOut()
      localStorage.removeItem('cafe_no_remember')
    }
  }, [])

  // ── Tab-Wechsel: Session bei Rückkehr prüfen ─────────────────────────────
  useEffect(() => {
    function onVisible() {
      if (document.visibilityState === 'visible') {
        supabase.auth.getSession().then(({ data }) => {
          if (!data.session) {
            // Session abgelaufen → Seite neu laden → zeigt Login
            setSession(null)
            setProfile(null)
            setLoading(false)
          }
  ...
  // ── Ladezustand ─────────────────────────────────────────────
  // Nur beim ERSTEN Laden Spinner zeigen — Hintergrund-Refreshes (Token-Refresh)
  // sollen die App nicht unmounten und damit keinen Tab-State-Reset verursachen
  ...
  const ctx = {
    ...
    // Convenience: name from profile OR from linked employee (resolved in Dashboard)
          ...
          <BrowserRouter>
            <div className="app-shell">
              ...
              <div className="main">
                ...
                {/* ── Auto-Logout: Abgelaufen-Hinweis ── */}
                {(() => {
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ display:'flex', gap:10, justifyContent:'center', flexWrap:'wrap' }}>
                    ...
                    <button
                      className="btn"
                      style={{ minWidth:140, color:'var(--danger)', border:'1px solid var(--danger)' }}
