                          ...
                          return (
                            <td key={i} className={`shift-cell${isTd?' today':isWe?' weekend':''}`} style={{
                              ...
                              {dayShifts.length === 0
                                ...
                                : dayShifts.map(s => (
                                  <div key={s.id}
                                    className={`shift-pill${s.employee_id===myEmpId?' mine':''}`}
                                    style={{ ...colors, margin:'2px 0', display:'block', cursor: canEdit ? 'pointer' : 'default' }}
                                    title={canEdit ? 'Klicken zum Bearbeiten' : `${DAY_FULL[i]} · ${s.start_time?.slice(0,5)}–${s.end_time?.slice(0,5)}`}
                                      onClick={canEdit ? () => openEditModal(s) : undefined}
                                  >
