 ...
 * Fristen und Prüfung liegen serverseitig (Migration 11_retention).
 * Reihenfolge beim Löschen: erst Dateien (Storage-API), dann Datensätze.
 */
const fmtDate = d => d ? new Date(d + 'T00:00:00').toLocaleDateString('de-DE', { day:'2-digit', month:'2-digit', year:'numeric' }) : ''

async function removeFiles(files) {
  const byBucket = {}
  files.forEach(f => { (byBucket[f.bucket] = byBucket[f.bucket] || []).push(f.name) })
  let failed = 0
  for (const [bucket, names] of Object.entries(byBucket)) {
    for (let i = 0; i < names.length; i += 100) {
      ...
      else failed += chunk.length - (data?.length ?? chunk.length)
    }
  }
  return failed
}
...
export default function RetentionCard() {
  ...
  const [confirm, setConfirm] = useState(null)   // Kategorie
  ...
  async function purge() {
    ...
    try {
      ...
      // 2. Dateien, 3. Datensätze
      const failed = confirm.files?.length ? await removeFiles(confirm.files) : 0
      const { data, error } = await supabase.rpc('retention_purge', { p_cat: confirm.key })
      if (error || !data?.success) throw new Error(data?.error || 'Löschen fehlgeschlagen.')
      const files = (confirm.files?.length || 0) - failed
      const parts = []
      if (data.deleted) parts.push(`${data.deleted} Einträge`)
      if (files) parts.push(`${files} Dateien`)
      if (failed || (data.skipped && confirm.key !== 'verwaist')) {
        toast.warn(`Teilweise gelöscht${parts.length ? ` (${parts.join(', ')})` : ''}. Einige Dateien konnten nicht entfernt werden – bitte später erneut versuchen.`, 9000)
      } else {
        toast.success(`🗑️ Gelöscht: ${parts.join(', ') || 'nichts mehr fällig'}`)
      }
    ...
    <div className="card" style={{ marginTop:16 }} id="aufbewahrung">
      ...
      <div className="card-body">
        <div style={{ fontSize:13, color:'var(--text-secondary)', lineHeight:1.6, marginBottom:12 }}>
          ...
          Hier siehst du, was die Frist erreicht hat. Gelöscht wird erst, wenn du es bestätigst.
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {cats.map(c => (
            <div key={c.key} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', border:'1px solid var(--border)', borderRadius:10, flexWrap:'wrap', background: c.due > 0 ? 'var(--warn-light, #FFF7ED)' : 'transparent' }}>
              <div style={{ flex:1, minWidth:200 }}>
                ...
                <div style={{ fontSize:12, marginTop:2, color: c.due > 0 ? 'var(--warn)' : 'var(--text-secondary)' }}>
                  {c.due > 0
                    ? `${c.due} ${c.key === 'verwaist' ? 'Datei(en)' : c.key === 'ehemalige' ? 'Person(en)' : 'Einträge'} fällig${c.key === 'ehemalige' && c.names?.length ? `: ${c.names.join(', ')}` : ''}`
                    : c.next_due ? `Nichts fällig – frühestens ab ${fmtDate(c.next_due)}` : 'Nichts fällig'}
                </div>
              </div>
        ...
        <div className="modal-overlay" onClick={() => !busy && setConfirm(null)}>
          <div className="modal" style={{ maxWidth:460 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ fontSize:13.5, lineHeight:1.6 }}>
              <strong>{confirm.title}</strong>: {confirm.due} {confirm.key === 'verwaist' ? 'Datei(en)' : confirm.key === 'ehemalige' ? 'Person(en) mit allen Daten' : 'Einträge'}
              {confirm.files?.length > 0 && confirm.key !== 'verwaist' && <> und {confirm.files.length} Datei(en)</>} werden endgültig gelöscht.
              {confirm.key === 'ehemalige' && confirm.names?.length > 0 && <div style={{ marginTop:6 }}>Betrifft: {confirm.names.join(', ')}</div>}
              <div style={{ fontSize:12.5, color:'var(--text-secondary)', marginTop:8 }}>Das lässt sich nicht rückgängig machen.</div>
              <label style={{ display:'flex', gap:10, alignItems:'flex-start', marginTop:12, cursor:'pointer' }}>
                ...
                <span>Ich habe vorher eine aktuelle Datensicherung heruntergeladen (oder brauche die Daten sicher nicht mehr).</span>
              </label>
            </div>
